...
int buscaBinaria(vector <Registro> &vec, string substr){
	...
	while(izq <=der){
    	...
    	if(comparacion == 0){
			cout<<med<<endl;
			if(vec[med].sububi.compare(vec[med-1].sububi)!=0){
				return med;
			}
			der = med;
		}
...
int main(){
	...
	string serie;
	cout<<"Indique el UBI:"<<endl;
	getline(cin,serie);
	// Busqueda Binaria
	int pos = buscaBinaria(vectRegistro,serie);
	cout<<pos<<endl;
	if(pos==-1){
		cout<<"El UBI no existe"<<endl;
	}
	while(vectRegistro[pos].sububi==serie){
		cout<<vectRegistro[pos].fecha<<" - "<<vectRegistro[pos].hora<<" - "<<vectRegistro[pos].entrada<<" - "<<vectRegistro[pos].ubi<<endl;
		pos++;
	}
